...
const skillCategories = [
  { name: 'Frontend', icon: '🎨', skills: [{ name: 'React / Next.js', level: 95 },{ name: 'TypeScript', level: 85 },{ name: 'Tailwind CSS', level: 90 },{ name: 'Framer Motion', level: 80 }] },
  { name: 'Backend', icon: '⚙️', skills: [{ name: 'Node.js / Express', level: 92 },{ name: 'Java / Spring Boot', level: 78 },{ name: 'REST & GraphQL APIs', level: 88 },{ name: 'WebSockets', level: 75 }] },
  { name: 'Database', icon: '🗄️', skills: [{ name: 'MongoDB / Mongoose', level: 88 },{ name: 'PostgreSQL', level: 82 },{ name: 'Redis', level: 70 },{ name: 'Elasticsearch', level: 65 }] },
  { name: 'DevOps & Tools', icon: '🚀', skills: [{ name: 'Docker / Kubernetes', level: 75 },{ name: 'AWS / GCP', level: 72 },{ name: 'CI/CD (GitHub Actions)', level: 80 },{ name: 'Linux / Bash', level: 85 }] },
];

const experiences = [
  { title: 'Senior Full-Stack Developer', company: 'TechCorp Solutions', period: '2022 – Present', description: 'Lead developer on a team of 6, architecting microservices and React dashboards for enterprise clients. Reduced API response time by 60% through caching and query optimization.', tech: ['React', 'Node.js', 'MongoDB', 'Docker', 'AWS'] },
  { title: 'Full-Stack Developer', company: 'Startup XYZ', period: '2020 – 2022', description: 'Built and shipped a SaaS analytics platform from scratch. Implemented real-time dashboards with WebSockets and designed the entire backend architecture.', tech: ['Vue.js', 'Express', 'PostgreSQL', 'Socket.io'] },
  { title: 'Junior Web Developer', company: 'Digital Agency', period: '2019 – 2020', description: 'Developed responsive websites and e-commerce platforms for 20+ clients. Gained deep experience in frontend performance optimization.', tech: ['HTML/CSS', 'JavaScript', 'WordPress', 'PHP'] },
];

const defaultCareerJourney = [
  { year: '2018', title: 'Beginning the Journey', subtitle: 'Learning by building', description: 'Started exploring web development fundamentals, creating simple pages, and discovering how design, logic, and interactivity work together on the web.', accent: 'from-primary-500 to-sky-400' },
  { year: '2019', title: 'Building the Basics', subtitle: 'Practice and consistency', description: 'Moved past tutorials into real project practice, improving HTML, CSS, JavaScript, and responsive thinking through hands-on experimentation and iteration.', accent: 'from-sky-500 to-emerald-400' },
  { year: '2020', title: 'Going Full Stack', subtitle: 'Frontend met backend', description: 'Expanded from interface work into APIs, databases, and deployment, learning how to connect polished user experiences with reliable backend systems.', accent: 'from-emerald-500 to-lime-400' },
  { year: '2022', title: 'Professional Growth', subtitle: 'Shipping for real users', description: 'Focused more on production-grade development, cleaner architecture, performance, reusable components, and building products with practical business impact.', accent: 'from-amber-400 to-orange-500' },
  { year: 'Now', title: 'Career With Purpose', subtitle: 'Modern product engineering', description: 'Continuing to grow as a full-stack developer by creating scalable products, refining system thinking, and delivering experiences that feel intentional and useful.', accent: 'from-fuchsia-500 to-primary-500' },
];
...
const ProfilePhoto = ({ url, name = 'Your Name' }) => {
  const [imgError, setImgError] = useState(false);
  const initials = name.split(' ').map((w) => w[0]).join('').slice(0, 2).toUpperCase();
  return (
    <div className="relative flex items-center justify-center">
      {/* Spinning dashed ring */}
      <svg className="absolute w-[340px] h-[340px] lg:w-[420px] lg:h-[420px] text-primary-300/40 dark:text-primary-700/40" viewBox="0 0 200 200">
        <motion.circle cx="100" cy="100" r="96" fill="none" stroke="currentColor" strokeWidth="1.5"
          strokeDasharray="8 5" animate={{ rotate: 360 }}
      ...
      {/* Badge — experience */}
      <motion.div initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.9, type: 'spring' }}
        className="absolute top-0 -left-2 z-20 bg-primary-600 rounded-2xl px-3 py-2 shadow-xl">
        <span className="text-white text-sm font-bold block">5+</span>
...
const AboutPage = () => {
  ...
  const displayBio = bio
    ...
    : [
        ...
        "My approach combines pragmatic engineering with a keen eye for UX. I believe great software isn't just functional — it's fast, maintainable, and delightful to use.",
  ...
  return (
    <div>
      ...
      <Section className="pt-24 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}
            ...
            <ProfilePhoto url={profilePicUrl} name={displayName} />
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.15 }}>
            ...
            <div className="grid grid-cols-2 gap-3 mt-6 mb-8">
              {[{ label: 'Name', value: displayName },{ label: 'Email', value: email || 'your@email.com' },{ label: 'Location', value: location || 'Your City, Country' },{ label: 'Role', value: headline || 'MERN / TypeScript' }].map((item) => (
      ...
      {/* ── SKILLS ─────────────────────────────────────────────────────── */}
      <Section className="bg-slate-50 dark:bg-slate-900/50" title="Technical Skills" subtitle="Technologies I use to bring ideas to life">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {skillCategories.map((category, i) => (
            <motion.div key={category.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="card p-6">
              <h3 className="font-display font-semibold text-lg text-slate-900 dark:text-white mb-5 flex items-center gap-2">
                <span>{category.icon}</span> {category.name}
              </h3>
              {category.skills.map((skill) => <SkillBar key={skill.name} {...skill} />)}
            </motion.div>
          ))}
        </div>
      </Section>
                ...
                <div className="pl-10 md:pl-0 md:w-1/2">
                  <div className="card p-6">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className="text-xs font-mono text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded whitespace-nowrap">{exp.period}</span>
                    </div>
                    <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed mb-4">{exp.description}</p>
                    <div className="flex flex-wrap gap-1.5">{exp.tech.map((t) => <span key={t} className="tag">{t}</span>)}</div>
                  </div>
                </div>
              </motion.div>
            ))}
                ...
                <div className={index % 2 === 0 ? '' : 'md:order-1'}>
                  <div className="relative card overflow-hidden p-6 md:p-7">
                    ...
                    <div className="mb-4 flex items-center justify-between gap-3">
                      <span className={`inline-flex rounded-full bg-gradient-to-r px-3 py-1 text-xs font-mono font-semibold text-white shadow-lg ${item.accent}`}>
                        {item.year}
                      </span>
      ...
      <Section id="resume" className="bg-slate-50 dark:bg-slate-900/50" title="My Resume" subtitle="A snapshot of my qualifications and career highlights">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="card overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-5">
            ...
            <div className="lg:col-span-2 gradient-bg p-10 flex flex-col items-center justify-center text-white relative overflow-hidden min-h-[280px]">
              ...
              <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 3.5, ease: 'easeInOut' }} className="relative z-10">
                {/* Animated PDF icon */}
                <svg className="w-28 h-28 drop-shadow-2xl" viewBox="0 0 80 80" fill="none">
                  ...
                  <path d="M60 55v8m0 0l-3.5-3.5M60 63l3.5-3.5" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
              ...
              <p className="relative z-10 text-white/70 text-sm mt-1.5 font-mono">Your Name · Full-Stack Dev</p>
            </div>
            ...
            <div className="lg:col-span-3 p-8 lg:p-10 flex flex-col justify-center">
              ...
              <ul className="space-y-2 mb-8">
                {['5+ years full-stack engineering', 'MERN stack & cloud architecture (AWS)', 'Open-source contributor', 'AWS Certified Developer Associate'].map((item) => (
              ...
              {hasResume && resumeUrl ? (
                <div className="flex flex-wrap gap-3">
                  <a href={resumeUrl} download target="_blank" rel="noopener noreferrer" className="btn-primary">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                    Download PDF
                  </a>
