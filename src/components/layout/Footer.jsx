...
const Footer = () => {
  ...
  const { siteTitle, logoLetter, footerTagline, githubUrl, linkedinUrl, twitterUrl, email, fullName } = useProfile();

  const socials = [
    { label: 'GitHub', href: githubUrl, icon: (
    ...
    { label: 'Twitter', href: twitterUrl, icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    <footer className="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <span className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center text-white text-sm font-bold">{(logoLetter || 'P').slice(0, 2)}</span>
              <span className="font-display font-bold text-lg text-slate-900 dark:text-white">{siteTitle || 'Portfolio'}</span>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
              {footerTagline || 'Full-stack developer passionate about building scalable, user-centric applications.'}
            </p>
          </div>
          ...
          <div>
            ...
            <p className="mt-4 text-sm text-slate-400">{email || 'your.email@example.com'}</p>
          </div>
        </div>
        ...
        <div className="mt-8 pt-8 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-sm text-slate-400">&copy; {year} {fullName || siteTitle || 'Portfolio'}. All rights reserved.</p>
